<template>
  <div>
    <field-header
        :title="`${fieldIndex + 1} Questionnaire Task`"
    ...
    <div v-else>
      ...
      <div class="-mx-4 p-4 border-b">
        ...
        <div class="mb-2">
          <label class="input-label">Add the forms below:</label>
          <div class="mb-2 flex items-center w-full" v-for="(form, formIndex) in proxyValue.forms">
            <div class="relative flex items-center w-full">
              <select class="input" v-model="proxyValue.forms[formIndex]">
                <option v-for="form in forms" :key="form.id" :value="form.id">{{ form.name }}</option>
                <option v-if="forms.length === 0" :value="null" selected disabled>You haven't created any forms yet.</option>
              </select>
              <i class="fas fa-caret-down absolute right-0 mr-4"></i>
            </div>
            <button class="ml-2 btn btn-gray-text" @click="removeForm(formIndex)"><i class="fas fa-trash"></i></button>
            <button class="ml-2 btn btn-gray-text" @click="moveFormUp(formIndex)"><i class="fas fa-arrow-up"></i></button>
            <button class="ml-2 btn btn-gray-text" @click="moveFormDown(formIndex)"><i class="fas fa-arrow-down"></i></button>
          </div>
          <div>
            <button class="btn btn-green-outline" @click="addForm"><i class="fas fa-plus"></i> Add form</button>
          </div>
...
export default {
  ...
  methods: {
    addForm() {
      this.proxyValue.forms.push({
        id: 1,
      });
    },
    removeForm(index) {
      this.proxyValue.forms.splice(index, 1);
    },
    moveFormUp(index) {
      const tmp = JSON.parse(JSON.stringify(this.proxyValue.forms[index]));
      this.removeForm(index);
      this.proxyValue.forms.splice(index - 1, 0, tmp);
    },
    moveFormDown(index) {
      const tmp = JSON.parse(JSON.stringify(this.proxyValue.forms[index]));
      this.removeForm(index);
