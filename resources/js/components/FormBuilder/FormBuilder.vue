<template>
    <div>
        <tabs class="mt-4">
            <tab :selected="true" :name="tabName('Questionnaire', form)">
                <questionnaire-builder v-model="form.fields"></questionnaire-builder>
            </tab>
            <tab :name="tabName('Task', task)">
                <task-slot v-model="task" :forms-path="projectPath + '/forms'"></task-slot>
            </tab>
        </tabs>
        <transition name="slide-fade">
            <div v-if="isDirty" class="fixed bottom-0 right-0 flex items-center justify-between p-4 bg-white rounded-tl-lg shadow">
                <div>
